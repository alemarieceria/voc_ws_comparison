...
6. **Export:** `exported_ws_data` writes the final shapefile to `/data/03_output/merged_water_samples/`.

Each circle in the graph represents a target (a single object or file),
and each square represents **dynamic branches** that run independently for each layer. See `R/` folder in repository for scripts used in pipeline.
